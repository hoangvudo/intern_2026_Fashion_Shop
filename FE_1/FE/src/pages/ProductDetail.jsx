...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setProduct(data)
        // Ảnh đầu tiên từ images hoặc thumbnailUrl
  ...
  // Realtime stock từ API (cập nhật khi variant thay đổi)
  ...
  useEffect(() => {
    ...
    const fetchStock = async () => {
      try {
        ...
        const res = await fetch(`/api/products/${product.id}/stock?${params}`)
  ...
  const currentStock = liveStock !== null ? liveStock : (selectedVariant?.stock ?? product?.totalStock ?? 0)
  ...
  // Tất cả ảnh của sản phẩm (thumbnail + gallery)
  const allImages = product
    ? [
        ...(product.thumbnailUrl ? [{ url: product.thumbnailUrl }] : []),
        ...(product.images ?? []).filter((img) => img.url !== product.thumbnailUrl),
      ]
    : []
