...
import com.fashion.backend.service.PayOSService;
...
public class OrderController {
     ...
     * Auth: required
     * ───────────────────────────────────────────────────────── */
    @PostMapping("/{id}/payos/create")
    public ResponseEntity<PayOSResponse> createPayOSLink(@PathVariable Long id) {
        return ResponseEntity.ok(payOSService.createPaymentLink(id));
    }

    /* ─────────────────────────────────────────────────────────
     * [PAYOS] WEBHOOK – Xử lý thông báo thanh toán từ PayOS
     * POST /api/orders/payos/webhook
     * Auth: không cần (validate signature trong service)
     * ───────────────────────────────────────────────────────── */
    @PostMapping("/payos/webhook")
    public ResponseEntity<Map<String, Object>> payOSWebhook(
            @RequestBody String webhookBody,
            @RequestHeader(value = "x-signature", required = false) String signature) {
        return ResponseEntity.ok(payOSService.handleWebhook(webhookBody, signature));
    }
    ...
    public ResponseEntity<Map<String, Object>> checkPayOSStatus(@PathVariable Long id) {
        return ResponseEntity.ok(payOSService.checkPaymentStatus(id));
    }
}
