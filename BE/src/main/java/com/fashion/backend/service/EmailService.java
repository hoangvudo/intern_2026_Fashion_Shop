...
public class EmailService {
    ...
    public void sendOrderConfirmationEmail(com.fashion.backend.entity.Order order) {
        String toEmail = order.getUser().getEmail();
        String fullName = order.getUser().getFullName();
        String subject = "[Fashion Shop] Xác nhận đơn hàng #" + order.getOrderCode();
        
        StringBuilder itemsHtml = new StringBuilder();
        for (com.fashion.backend.entity.OrderItem item : order.getItems()) {
            itemsHtml.append(String.format("""
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #ddd;">
                        <img src="%s" width="60" style="border-radius: 4px;"/>
                    </td>
                    <td style="padding: 10px; border-bottom: 1px solid #ddd;">
                        <strong>%s</strong><br/>
                        <span style="color:#666; font-size:12px;">Size: %s | Màu: %s</span>
                    </td>
                    <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: center;">%d</td>
                    <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: right;">%,.0f₫</td>
                </tr>
                """, 
        ...
        String body = """
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
                ...
                <p>Xin chào <strong>%s</strong>,</p>
                <p>Cảm ơn bạn đã mua sắm tại Fashion Shop. Đơn hàng <strong>#%s</strong> của bạn đã được ghi nhận.</p>
                
                <h3 style="border-bottom: 2px solid #eee; padding-bottom: 5px;">Thông tin giao hàng</h3>
                <p>
                    <strong>Người nhận:</strong> %s<br/>
                    <strong>SĐT:</strong> %s<br/>
                    <strong>Địa chỉ:</strong> %s<br/>
                    <strong>Phương thức thanh toán:</strong> %s
                </p>
                ...
                <div style="text-align: right; margin-top: 20px; font-size: 16px;">
                    <p>Tạm tính: <strong>%,.0f₫</strong></p>
                    <p>Phí ship: <strong>%,.0f₫</strong></p>
                    <p>Giảm giá: <strong>-%,.0f₫</strong></p>
                    <h3 style="color: #d32f2f;">Tổng cộng: %,.0f₫</h3>
                </div>

                <div style="text-align:center; margin: 30px 0;">
                    <a href="%s/user/orders" style="background:#2563eb;color:#fff;padding:12px 28px; border-radius:6px;text-decoration:none;font-weight:bold">Xem đơn hàng của bạn</a>
                </div>
            </div>
            """.formatted(
                fullName, 
