...
public class OrderService {
    ...
    public OrderResponse placeOrder(OrderRequest req) {
        ...
        try {
            ...
            log.error("[Order] Lỗi gửi email xác nhận cho đơn hàng {}", saved.getOrderCode(), e);
        }
    ...
    private void validateAndDeductStock(List<OrderRequest.OrderItemRequest> items) {
        ...
        for (OrderRequest.OrderItemRequest item : items) {
            ...
            allVariants.stream()
                    .filter(v -> v.getProduct().getId().equals(item.getProductId()) && matchVariant(v, item.getSize(), item.getColor()))
                    .findFirst()
                    .ifPresent(variant -> {
    ...
    private void restoreStock(List<OrderItem> items) {
        ...
        for (OrderItem item : items) {
            ...
            allVariants.stream()
                    .filter(v -> v.getProduct().getId().equals(item.getProduct().getId()) && matchVariant(v, item.getSize(), item.getColor()))
                    .findFirst()
                    .ifPresent(variant -> {
                        variant.setStock(variant.getStock() + item.getQuantity());
                        variantsToUpdate.add(variant);
    ...
    private OrderResponse toResponse(Order order) {
        ...
        OrderReturnResponse returnRequestInfo = null;
        List<OrderReturn> returns = orderReturnRepository.findByOrderIdOrderByCreatedAtDesc(order.getId());
        if (returns != null && !returns.isEmpty()) {
            OrderReturn r = returns.get(0);
            returnRequestInfo = OrderReturnResponse.builder()
                .id(r.getId())
