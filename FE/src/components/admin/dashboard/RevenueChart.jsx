...
function RevenueChart({ data = [], period, onPeriodChange, loading }) {
  ...
  const { path, areaPath, points, maxVal, yLabels } = useMemo(() => {
    // FIX: BE trả về { date, revenue, profit } — dùng revenue
    const vals = data.map(d => Number(d.revenue ?? d.value ?? d.amount ?? 0))
    if (vals.length < 2) return { path: null, areaPath: null, points: [], maxVal: 0, yLabels: [] }
    ...
    return { path: linePath, areaPath: aPath, points: pts, maxVal: max, yLabels: steps }
  }, [data])
  ...
  const xLabels = useMemo(() => {
    ...
    return data
      ...
      .map((d, i) => {
        const idx = i * step
        const x = PAD.left + (idx / Math.max(data.length - 1, 1)) * (W - PAD.left - PAD.right)
    ...
    <div className="border border-[#D1C4B9] bg-white p-8">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="font-beVietnamPro text-sm text-[#4E453D] opacity-70">Đơn vị: VNĐ</p>
        </div>
              ...
              {PERIODS.map(p => (
                <button
                  ...
                  className={`block w-full px-4 py-2.5 text-left font-beVietnamPro text-sm hover:bg-[#F0EEE9] ${
                    period === p.value ? 'font-semibold text-[#6F583D]' : 'text-[#1B1C19]'
                  }`}
                >
                  {p.label}
                </button>
          ...
          >
            ...
            {yLabels.map((l, i) => (
              <g key={i}>
                ...
                <text x={PAD.left} y={l.y - 3}
                  className="fill-[#9E8E7E]" fontSize="9" textAnchor="start">
                  {l.label}
                </text>
              </g>
            ))}
            ...
            {/* Line */}
            <path d={path} stroke="#6F583D" strokeWidth="2" fill="none"
            ...
            {tooltip && (
              <g>
                <rect
                  ...
                  y={tooltip.y - 40}
                  width="88" height="28" rx="4"
                  fill="#1B1C19" opacity="0.85"
                />
                <text
                ...
                <text
                  x={Math.min(tooltip.x - 40, W - 90) + 44}
                  y={tooltip.y - 10}
                  fill="#D1C4B9" fontSize="10" textAnchor="middle"
                >
            ...
            {xLabels.map((l, i) => (
              <text key={i} x={l.x} y={H - 4}
                fill="#9E8E7E" fontSize="9" textAnchor="middle">
                {l.label}
              </text>
            ))}
          </svg>
