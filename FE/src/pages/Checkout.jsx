...
/* ─── Dữ liệu địa chỉ tĩnh (mở rộng sau bằng API VAPI) ─── */
const ADDRESS_DATA = {
  'Hà Nội': {
    'Ba Đình': ['Phúc Xá', 'Trúc Bạch', 'Vĩnh Phúc', 'Cống Vị', 'Liễu Giai'],
    'Hoàn Kiếm': ['Phúc Tân', 'Đồng Xuân', 'Hàng Mã', 'Hàng Buồm', 'Lý Thái Tổ'],
    'Đống Đa': ['Khâm Thiên', 'Ô Chợ Dừa', 'Nam Đồng', 'Trung Phụng', 'Quang Trung'],
  },
  'TP. Hồ Chí Minh': {
    'Quận 1': ['Bến Nghé', 'Bến Thành', 'Cầu Kho', 'Cầu Ông Lãnh', 'Tân Định'],
    'Quận 3': ['Phường 1', 'Phường 2', 'Phường 3', 'Phường 4', 'Phường 5'],
    'Bình Thạnh': ['Phường 1', 'Phường 2', 'Phường 3', 'Phường 6', 'Phường 12'],
  },
  'Đà Nẵng': {
    'Hải Châu': ['Hải Châu 1', 'Hải Châu 2', 'Nam Dương', 'Phước Ninh', 'Thanh Bình'],
    'Sơn Trà': ['An Hải Bắc', 'An Hải Đông', 'Mân Thái', 'Nại Hiên Đông', 'Thọ Quang'],
  },
}
...
function CheckoutPage() {
  ...
  /* ── Địa chỉ dynamic ────────────────────────────────────── */
  const districts = Object.keys(ADDRESS_DATA[form.city] ?? {})
  const wards     = ADDRESS_DATA[form.city]?.[form.district] ?? []
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    <Field label="Thành phố *" error={errors.city}>
                      <select value={form.city} onChange={e => setField('city', e.target.value)} className={selectCls(errors.city)}>
                        {Object.keys(ADDRESS_DATA).map(c => <option key={c}>{c}</option>)}
                      </select>
                    </Field>
                    <Field label="Quận / Huyện *" error={errors.district}>
                      <select value={form.district} onChange={e => setField('district', e.target.value)} className={selectCls(errors.district)}>
